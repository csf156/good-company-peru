...
> - **TAREA 1 DE F.2 — `crear_invitacion` se traga la violación del índice.** Su `exception when unique_violation` estaba pensado para la idempotencia por `idempotency_key`, pero Postgres no distingue qué índice disparó la excepción: una segunda propuesta activa en el par devuelve **éxito con un registro de puros NULL y nada creado** (el `return` corta antes del insert en `ordenes_pago`, así que tampoco hay orden ni retención). Hay que acotar el manejador al constraint de idempotencia por nombre (`constraint_name` de `PG_EXCEPTION_DETAIL`) y dejar que el índice del par salga como error propio. El test de fijación de F.1 (36:912-925) pinea el comportamiento actual: **en F.2 se da vuelta** para exigir el error. Verificado en vivo contra `pg_get_functiondef` (2026-09-22).
